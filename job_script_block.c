...
static int lua_GetDurability(lua_State* L) {
    lua_pushnumber(L, luaMP_checkblock(L, 1, 1)->durability);
    return 1;
}

static int lua_GetGold(lua_State* L) {
    lua_pushinteger(L, luaMP_checkblock(L, 1, 1)->gold);
    return 1;
}

static int lua_GetOwner(lua_State* L) {
    lua_pushinteger(L, luaMP_checkblock(L, 1, 1)->owner);
    return 1;
}

static int lua_GetPassability(lua_State* L) {
    lua_pushinteger(L, luaMP_checkblock(L, 1, 1)->meta->passability);
    return 1;
}

static int lua_GetPosition(lua_State* L) {
    unsigned short x, y;
    if (!MP_GetBlockCoordinates(&x, &y, luaMP_checkblock(L, 1, 1))) {
...
static int lua_GetRoom(lua_State* L) {
    luaMP_pushroom(L, luaMP_checkblock(L, 1, 1)->room);
    return 1;
}

static int lua_GetStrength(lua_State* L) {
    lua_pushnumber(L, luaMP_checkblock(L, 1, 1)->strength);
    return 1;
}

static int lua_GetType(lua_State* L) {
    lua_pushinteger(L, luaMP_checkblock(L, 1, 1)->meta->id);
    return 1;
}

static int lua_IsPassable(lua_State* L) {
    lua_pushboolean(L, MP_IsBlockPassable(luaMP_checkblock(L, 1, 1)));
    return 1;
}
...
static int lua_IsDestructible(lua_State* L) {
    lua_pushboolean(L, MP_IsBlockDestructible(luaMP_checkblock(L, 1, 1)));
    return 1;
}

static int lua_IsConvertible(lua_State* L) {
    lua_pushboolean(L, MP_IsBlockConvertible(luaMP_checkblock(L, 1, 1)));
    return 1;
}
...
MP_Block* luaMP_checkblock(lua_State* L, int narg, int errarg) {
    ...
    luaL_argcheck(L, ud != NULL, errarg, "'" LUA_BLOCKLIBNAME "' expected");
    return *(MP_Block**) ud;
}
